...
from common import ReadJSON, WriteJSON, WriteLog, GetPythonExecutable
...
class MQTTBridge:
    ...
    def handle_command(self, entity_type, entity_name, payload_str, forced_duration=None):
        ...
        try:
            ...
            # Resolve interpreter + working dir exactly like app.py does
            script_dir = os.path.dirname(os.path.abspath(__file__))
            python_exec = GetPythonExecutable(json_data)
            use_sudo = json_data['settings']['target_sys'] != 'None'
            ...
            if entity_type == 'zone':
                ...
                command = [python_exec, 'control.py', '-f', '-z', entity_name, '-d', str(duration)]
                if use_sudo:
                    command.insert(0, 'sudo')

                subprocess.Popen(command, cwd=script_dir, stderr=open('/tmp/mqtt_control.log', 'a'))
                ...
                command = [python_exec, 'control.py', '-f', '-s', entity_name]
                if use_sudo:
                    command.insert(0, 'sudo')
